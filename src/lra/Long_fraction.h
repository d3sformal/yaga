...
enum class State: unsigned char {
    /*
     * bit 0 - whether word part is valid or not
...
static_assert(std::is_same<state_t,unsigned char>::value, "Underlying value for Long_fraction inner type is not as expected");
...
private:
    ...
    {
        const_cast<Long_fraction *>(this)->ensure_mpq_valid();
    }
    ...
    friend inline void addition            (Long_fraction &, const Long_fraction &, const Long_fraction &);
    friend inline void substraction        (Long_fraction &, const Long_fraction &, const Long_fraction &);
    friend inline void multiplication      (Long_fraction &, const Long_fraction &, const Long_fraction &);
    friend inline void division            (Long_fraction &, const Long_fraction &, const Long_fraction &);
    friend inline void additionAssign      (Long_fraction &, const Long_fraction &);
    friend inline void substractionAssign  (Long_fraction &, const Long_fraction &);
    friend inline void multiplicationAssign(Long_fraction &, const Long_fraction &);
    friend inline void divisionAssign      (Long_fraction &, const Long_fraction &);
    friend Long_fraction gcd                (Long_fraction const &, Long_fraction const &);
    friend Long_fraction lcm                (Long_fraction const &, Long_fraction const &);
    friend Long_fraction fastrat_fdiv_q     (Long_fraction const & n, Long_fraction const & d);
    friend Long_fraction divexact           (Long_fraction const & n, Long_fraction const & d);
...
public:
    ...
    inline bool operator==(const Long_fraction& b) const;
    inline Long_fraction operator-() const;

    inline void negate();

    Long_fraction get_den() const {
    ...
    bool operator< ( const Long_fraction & b ) const { return compare(b) < 0; }
    bool operator> ( const Long_fraction & b ) const { return compare(b) > 0; }
    bool operator<=( const Long_fraction & b ) const { return compare(b) <= 0; }
    bool operator>=( const Long_fraction & b ) const { return compare(b) >= 0; }
    bool operator!=( const Long_fraction & b ) const { return !(*this == b); }
    ...
    inline Long_fraction ceil() const
    {
        if (isInteger())
            return *this;
        ...
        if (wordPartValid()) {
            ...
            return Long_fraction(mpz());
        }
    }
    inline Long_fraction floor() const
    {
        if (isInteger()) return *this;
        return ceil() - 1;
    }
    bool isWellFormed() const;
    Long_fraction operator+(const Long_fraction& b) const
    {
        Long_fraction dest;
        assert(isWellFormed());
    ...
    Long_fraction& operator+=(const Long_fraction& b)
    {
        assert(isWellFormed());
        assert(b.isWellFormed());
    ...
    inline Long_fraction inverse() const;
    ...
    Long_fraction operator%(const Long_fraction& d) {
...
struct Long_fractionHash {
    uint32_t operator() (const Long_fraction& s) const {
        return (uint32_t)s.getHashValue();
    }
};

inline std::ostream & operator<<(std::ostream & out, const Long_fraction & r)
{
    r.print(out);
    return out;
}
inline Long_fraction::Long_fraction(const Long_fraction& x) {
    if (x.wordPartValid()) {
        num = x.num;
        den = x.den;
        state = State::WORD_VALID;
...
inline Long_fraction::Long_fraction(Long_fraction &&other) noexcept : state{other.state}, num{other.num}, den{other.den}  {
...
inline Long_fraction& Long_fraction::operator=(const Long_fraction& x) {
    if (x.wordPartValid()) {
        num = x.num;
        den = x.den;
...
inline bool Long_fraction::operator==(const Long_fraction& b) const {
...
inline Long_fraction Long_fraction::operator-() const {
    if (this->wordPartValid() && num > WORD_MIN) {
        return Long_fraction(-num, den);
    } else {
        force_ensure_mpq_valid();
        Long_fraction x;
        x.mpq = pool.alloc();
...
inline void Long_fraction::negate() {
    if (this->wordPartValid() && num > WORD_MIN) {
        num = -num;
        setMpqPartInvalid();
    } else {
...
inline int Long_fraction::compare(const Long_fraction& b) const {
    if (this->wordPartValid() && b.wordPartValid()) {
        if (b.den == den) {
...
inline int Long_fraction::sign() const {
    if (wordPartValid()) {
...
Long_fraction lcm(integer a, integer b) {
    if (a == 0) return 0;
    if (b == 0) return 0;
    if (b > a)
        return Long_fraction(b / gcd(a, b)) * a;
    else
        return Long_fraction(a / gcd(a, b)) * b;
}

// Return 1 if |op1| > |op2|, -1 if |op1| < |op2|, and 0 if op1 = op2
inline int cmpabs(Long_fraction op1, Long_fraction op2)
{
    if (op1.sign() == -1)
        op1 = -op1;
    if (op2.sign() == -1)
...
inline bool Long_fraction::isWellFormed() const
{
    assert(wordPartValid() or not fitsWord());
    ...
    // Check that if both wordPartValid() and mpqPartValid(), they are the same number
}

inline bool Long_fraction::wordAndMpqEqual() const {
...
inline Long_fraction::Long_fraction(word n, uword d) : state{State::WORD_VALID} {
    assert(d > 0);
    if (n == 0) {
        num = 0;
        den = 1;
...
inline void addition(Long_fraction& dst, const Long_fraction& a, const Long_fraction& b) {
...
inline void substraction(Long_fraction& dst, const Long_fraction& a, const Long_fraction& b) {
    if (a.wordPartValid() && b.wordPartValid()) {
        if (b.num == 0) {
            dst.num = a.num;
            dst.den = a.den;
...
inline void multiplication(Long_fraction& dst, const Long_fraction& a, const Long_fraction& b) {
...
inline void division(Long_fraction& dst, const Long_fraction& a, const Long_fraction& b) {
    if (b.wordPartValid() && b.num == 1 && b.den == 1) {
        dst = a;
        return;
    }
    ...
    if (a.wordPartValid() && b.wordPartValid()) {
        ...
        // Note: dst and a or b might be the same Long_fraction.
...
inline double Long_fraction::get_d() const {
    if (wordPartValid()) {
...
inline void additionAssign(Long_fraction& a, const Long_fraction& b) {
    if (b.wordPartValid()) {
        if (b.num == 0) return;
        if (a.wordPartValid()) {
            if (b.den == 1) {
...
inline void substractionAssign(Long_fraction& a, const Long_fraction& b) {
    if (a.wordPartValid() && b.wordPartValid()) {
        uword common = gcd(a.den, b.den);
...
inline void multiplicationAssign(Long_fraction& a, const Long_fraction& b) {
    if (a.wordPartValid() && b.wordPartValid()) {
...
inline void divisionAssign(Long_fraction& a, const Long_fraction& b) {
    if (a.wordPartValid() && b.wordPartValid()) {
        uword common1 = gcd(absVal(a.num), absVal(b.num));
        uword common2 = gcd(a.den, b.den);
        word zn;
...
inline Long_fraction abs(Long_fraction& x) {
    if (x.sign() >= 0) {
        return x;
    } else {
        return -x;
...
inline Long_fraction Long_fraction_inverse(const Long_fraction& x) {
    return x.inverse();
}

inline bool isNegative(Long_fraction const& num) {
    return num.sign() < 0;
}

inline bool isPositive(Long_fraction const& num) {
    return num.sign() > 0;
}

inline bool isNonNegative(Long_fraction const& num) {
    return num.sign() >= 0;
}

inline bool isNonPositive(Long_fraction const& num) {
    return num.sign() <= 0;
}
