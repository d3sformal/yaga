...
std::vector<Clause> Linear_arithmetic::decide(Database&, Trail& trail, Variable var, TrailModelsSnapshot const& input_model) {
    ...
    for (auto&& watched_constr : constr)
    {
        ...
        {
            std::cout << "eval(models.owned(), cons) => " << (eval(models.owned(), cons).has_value() ? "has value" : "does not have value") << std::endl;
            std::cout << "eval(models.boolean(), cons.lit()) => " << (eval(models.boolean(), cons.lit()).has_value() ? "has value" : "does not have value") << std::endl;
            std::cout << "if the literal has value == models.boolean().is_defined(cons.lit().var().ord()) => " << (models.boolean().is_defined(cons.lit().var().ord()) ? "true" : "false") << std::endl;
