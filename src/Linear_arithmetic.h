...
public:
    ...
    /** Create a constraint from @p args or return an existing object that represents the same constraint.
     *
     ...
     */
    inline std::pair<Implied_value<Value_type>, Implied_value<Value_type>> find_bounds(Model<Value_type> const& model, int lra_var_ord) 
    { 
        return {bounds[lra_var_ord].lower_bound(model), 
                bounds[lra_var_ord].upper_bound(model)}; 
    }
...
private:
     ...
     * @return literal that represents the linear constraint
     */
    template <typename... Args> inline Constraint_type constraint(Model<Value_type> const& model, Args&&... args)
    {
        auto cons = constraints.make(std::forward<Args>(args)...);
        watch(cons, model);
        return cons;
     ...
     */
    std::optional<Clause> check_bounds(Model<bool> const& bool_model,
     ...
     */
    std::optional<Value_type> check_equality(Model<Value_type> const& lra_model, Bounds<Value_type>& bounds);

    /** Report a new unit constraint @p cons and check for conflicts/implied values.
     * 
     * If the set of appropriate values for the unassigned variable in @p cons becomes empty, this method returns a conflict. If the set becomes a singleton (i.e., exactly one value is permitted), it is propagated to @p trail
     * 
     * @param assigned stack with assigned variables (add only if bounds for the unassigned variable imply an equality)
     * @param trail current solver trail
     * @param bool_model partial assignment of boolean variables
     * @param cons new unit constraint
     */
    std::optional<Clause> unit(std::vector<int>& assigned, Trail& trail, Model<bool> const& bool_model, Model<Value_type>& lra_model, Constraint_type& cons);
     ...
     * @return conflict clause if a conflict is detected. None, otherwise.
     */
    std::optional<Clause> check_bound_conflict(Model<Value_type> const& lra_model,
                                               Bounds<Value_type>& bounds);
    ...
    /** Check whether the unit constraint @p cons implies an equality for the only unassigned variable (e.g., `x == 5`)
     * 
    ...
    /** Check whether the unit constraint @p cons implies an inequality for the only unassigned variable (e.g., `x != 4`)
     * 
     * @param cons linear constraint with exactly one unassigned variable
    ...
    /** Check whether the unit constraint @p cons implies a lower bound for the only unassigned variable (e.g., `x > 0`, or `x >= 0`)
     * 
     * @param cons linear constraint with exactly one unassigned variable
    ...
    /** Check whether the unit constraint @p cons implies an upper bound for the only unassigned variable (e.g., `x < 0`, or `x <= 0`)
     * 
     * @param cons linear constraint with exactly one unassigned variable
     ...
     */
    void propagate(Trail& trail, Model<bool>& bool_model, Model<Value_type> const& lra_model, Constraint_type const& cons);
