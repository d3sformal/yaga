...
TEST_CASE("Prefer bounds with assigned variables over bounds with bounded variables", "[bounds]")
{
    using namespace perun;
    using namespace perun::test;
    ...
    bounds[y.ord()].add_lower_bound(models, {y.ord(), 0, trail[1], models});
    REQUIRE(bounds[y.ord()].lower_bound(models));

    // deduce x <= 0 from x + y <= 0 and y >= 0
    bounds[x.ord()].add_upper_bound(models, {x.ord(), 0, trail[0], models,
        std::vector{*bounds[y.ord()].lower_bound(models)}});
    REQUIRE(bounds[x.ord()].upper_bound(models));

    // bound x using the unit constraint x + y <= 0 which should take precedence over the previous 
    // bound even though the bound value is the same
    models.owned().set_value(y.ord(), 0);
    bounds[x.ord()].add_upper_bound(models, {x.ord(), 0, trail[0], models});
    REQUIRE(bounds[x.ord()].upper_bound(models));
    REQUIRE(bounds[x.ord()].upper_bound(models)->value() == 0);
    REQUIRE(bounds[x.ord()].upper_bound(models)->reason().lit() == trail[0].lit());
    REQUIRE(bounds[x.ord()].upper_bound(models)->bounds().empty());
}